...
def read_routing_point_api_data(key, points):
    ...
    for point in points:
        if point == points[len(points) - 1]:
            ...
            str_points += str(point) + ":"
    path = "https://api.tomtom.com/routing/1/calculateRoute/" + str_points.replace(" ","") + "/json?key=" + key + "&traffic=true"
    return json_it(path)

# method of the coordinates

def right_time(timeZone, start_hour, end_hour, minutes, seconds):
    datetime_ABC = datetime.now(timeZone)
    return (end_hour > int(datetime_ABC.strftime("%H")) >= start_hour and int(
        datetime_ABC.strftime("%M")) % minutes == 0 and int(datetime_ABC.strftime("%S")) % seconds == 0)
...
df = pd.read_csv("traffics/data/roads_points2.csv", error_bad_lines=False)
...
if __name__ == '__main__':
    ...
    while flag == 1:
        if right_time(timeZone=tz_SDN, start_hour=10, end_hour=4, minutes=30, seconds=60):
            for index, instance in df.iterrows():
                ...
                if index % 21 == 0:
                    ...
                    if data == 0:#if the request is failed
                        flag = 4
                        print('breaked')
                        break
